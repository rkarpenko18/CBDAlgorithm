    ...
    class Program
    {
        ...
        public class Bus : IComparable<Bus>
        {
            ...
            public double timeTaken()
            {
                ...
                {
                    case (1):
                        answer = totalTime(groups[0]);
                        break;
                    case (2):
                        answer = totalTime(groups[0], groups[1]);
                        break;
                    case (3):
                        answer = totalTime(groups[0], groups[1], groups[2]);
                        break;
                    default:
                        answer = 0;
                        break;
        ...
        static int[] kmeansArr = new int[38];  //num of groups
        ...
        public static double totalTime(Group B)
        {
            ...
            //only combo (AB)
            return A.distTo[B.destination.address];
        }
        ...
        public static double totalTime(Group B, Group C)
        {
            ...
            //first combo (ABC)
            double ABC;
            ABC = A.distTo[B.destination.address] + B.destination.distTo[C.destination.address];
            if (ABC < minTime) minTime = ABC;
            //second combo (ACB)
            double ACB;
            ACB = A.distTo[C.destination.address] + C.destination.distTo[B.destination.address];
        ...
        public static double totalTime(Group B, Group C, Group D)
        {
            ...
            //ABCD
            double ABCD;
            ABCD = A.distTo[B.destination.address] + B.destination.distTo[C.destination.address] + C.destination.distTo[D.destination.address];
            if (ABCD < minTime) minTime = ABCD;
            //ABDC
            double ABDC;
            ABDC = A.distTo[B.destination.address] + B.destination.distTo[D.destination.address] + D.destination.distTo[C.destination.address];
            if (ABDC < minTime) minTime = ABDC;
            //ACBD
            double ACBD;
            ACBD = A.distTo[C.destination.address] + C.destination.distTo[B.destination.address] + B.destination.distTo[D.destination.address];
            if (ACBD < minTime) minTime = ACBD;
            //ACDB
            double ACDB;
            ACDB = A.distTo[C.destination.address] + C.destination.distTo[D.destination.address] + D.destination.distTo[B.destination.address];
            if (ACDB < minTime) minTime = ACDB;
            //ADBC
            double ADBC;
            ADBC = A.distTo[D.destination.address] + D.destination.distTo[B.destination.address] + B.destination.distTo[C.destination.address];
            if (ADBC < minTime) minTime = ADBC;
            //ADCB
            double ADCB;
            ADCB = A.distTo[D.destination.address] + D.destination.distTo[C.destination.address] + C.destination.distTo[B.destination.address];
        ...
        public static void putAccessibleGroupsOnBuses()
        {
            ...
            if (aGroups.Count() > 0)
            {
                ...
                if (aGroups[0].numStudents <= aBuses[0].seatsRemaining(potentialBuses, -1, -1, -1))
                {
                ...
                for (int bus = 0; bus < aBuses.Count(); bus++)
                {
                    ...
                    for (int g = 0; g < aGroups.Count(); g++)
                    {
                        ...
                        if (assignments[g] == -1)
                        {
                            ...
                            if (travelTime <= 40 && aGroups[g].numStudents <= aBuses[bus].seatsRemaining(potentialBuses, -1, -1, -1))
                            {
                ...
                for (int cb = 0; cb < clusters[cl].combos.Count(); cb++)
                {
                    ...
                    for (int clump = 0; clump < clusters[cl].combos[cb].Count(); clump++)
                    {
                        ...
                        for (int bus = 0; bus < buses.Count(); bus++)
                        {
                            ...
                            if (buses[bus].filled == false && !busTaken(potentialBuses, bus) && (clusters[cl].combos[cb][clump].Count() + buses[bus].groups.Count()) <= 3)
                            {
                                ...
                                Console.Write("bus: " + bus + " timeOnThisPossibleBus " + timeOnThisPossibleBus + " clumpSum: " + clumpSum + " seatsRemaining: " + buses[bus].seatsRemaining(potentialBuses, bus, cl, cb));
                                if (clumpSum <= buses[bus].seatsRemaining(potentialBuses, bus, cl, cb) && timeOnThisPossibleBus <= 40)
                                {
        ...
        public static void outputBuses()
        {
            for (int bus = 0; bus < buses.Count(); bus++)
            {
                ...
                Console.WriteLine("Time taken: " + buses[bus].timeTaken());
                Console.Write("\n");
            }
        }
        ...
        static Group[] groups = new Group[38];                 //needs to be the right number of groups
        static Group[] aGroups = new Group[3];                  //needs to be number of accessible groups
        ...
        static string invalidSolString = "";
        ...
        static void Main(string[] args)
        {
            ...
            {
                Group[] groupArr = new Group[38] {  new Group(locations[0],-1,28,false),
                                                    new Group(locations[1],-1,16,false), new Group(locations[2],-1,14,false),new Group(locations[3],-1,23,false),new Group(locations[4],-1,24,false),new Group(locations[5],-1,17,false),
                                                    /*new Group(locations[6],-1,27,false),*/new Group(locations[7],-1,13,false),new Group(locations[8],-1,17,false),new Group(locations[9],-1,21,false),new Group(locations[10],-1,24,false),
                                                    new Group(locations[11],-1,13,false),new Group(locations[12],-1,21,false),new Group(locations[13],-1,40,false),new Group(locations[14],-1,19,false),new Group(locations[15],-1,27,false),
                                                    new Group(locations[16],-1,20,false),new Group(locations[17],-1,25,false),/*new Group(locations[18],-1,22,false),*/new Group(locations[19],-1,27,false),new Group(locations[20],-1,12,false),
                                                    new Group(locations[21],-1,13,false),new Group(locations[22],-1,12,false),new Group(locations[23],-1,13,false),new Group(locations[24],-1,21,false),new Group(locations[25],-1,27,false),
                                                    new Group(locations[26],-1,12,false),new Group(locations[27],-1,27,false),new Group(locations[28],-1,22,false),new Group(locations[29],-1,24,false),new Group(locations[30],-1,19,false),
                                                    /*new Group(locations[31],-1,22,false),*/new Group(locations[32],-1,8,false),new Group(locations[33],-1,26,false),new Group(locations[34],-1,28,false),new Group(locations[35],-1,24,false),
                                                    new Group(locations[36],-1,22,false), new Group(locations[37],-1,28,false), new Group(locations[38],-1,23,false), new Group(locations[39],-1,14,false), new Group(locations[40],-1,12,false)};
                for (int i = 0; i < 38; i++)
                {
                    groups[i] = groupArr[i];
                }
            ...
            {
                //originally 6,-1,27,true ... 18,-1,22,true ... 31,-1,22,true
                //alternate 6,-1,24,true ... 18,-1,31,true ... 31,-1,20,true
                Group[] aGroupArr = new Group[3] { new Group(locations[6], -1, 24, true), new Group(locations[18], -1, 31, true), new Group(locations[31], -1, 20, true) };
                for (int i = 0; i < 3; i++)
                {
                    aGroups[i] = aGroupArr[i];
                }
            }
            //initialize buses list
            {
                Bus[] busArr = new Bus[30] { new Bus(50,false), new Bus(50,false), new Bus(25,true), new Bus(50,false), new Bus(50,false), new Bus(50,false),
                                                new Bus(50,true), new Bus(50,false), new Bus(20,true), new Bus(50,false), new Bus(50,false), new Bus(50,false),
                                                new Bus(50,false), new Bus(50,false), new Bus(50,false), new Bus(50,false), new Bus(50,false), new Bus(50,false),
                                                new Bus(50,false), new Bus(50,false), new Bus(50,false), new Bus(50,false), new Bus(50,false), new Bus(50,false),
                                                new Bus(50,false), new Bus(50,false), new Bus(50,false), new Bus(50,false), new Bus(50,false), new Bus(50,false)};
            ...
            //initialize dictionary
            {
                double[,] dict = new double[41, 41] { {     0,   25,  4,   22,  31,  22,  18,  26,  10,  24,  9,   16,  27,  19,  19,  24,  23,  22,  36,  19,  23,  17,  24,  20,  17,  21,  22,  25,  14,  18,  17,  12,  14,  27,  20,  17,  19,  23,  4,   6,   23,   },
                                                    ...
                                                    {   18,  8,   18,  8,   18,  5,   0,   11,  23,  7,   21,  4,   11,  4,   4,   11,  6,   8,   23/*was 22 before*/,  5,   9,   4,   7,   6,   14,  8,   5,   7,   26,  12,  8,   11,  17,  15,  6,   3,   31,  8,   16,  17,  21,   },
                                                    ...
                                                    {   36,  20,  33,  19,  12,  20,  23/*was 22 before*/,  20,  28,  18,  37,  21,  19,  22,  21,  22,  18,  20,  0,   22,  22,  23,  19,  23,  25,  17,  19,  17,  32,  22,  26,  29,  23,  15,  21,  22,  29,  23,  32,  30,  16,   },
